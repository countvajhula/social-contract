     ...
     (test-suite
      ...
      (test-suite
       ...
       (check-equal? (translate (self-map/c procedure?))
                     'functional/c)
       (check-equal? (translate (binary-function/c a b (listof b)))
                     '(lift/c b listof (head a)))
       (check-equal? (translate (binary-function/c a b (listof a)))
                     '(lift/c a listof (tail b)))
       (check-equal? (translate (binary-function/c a any/c sequence?))
                     '(lift/c (head a)))
       (check-equal? (translate (binary-function/c any/c a sequence?))
                     '(lift/c (tail a)))
       (check-equal? (translate (binary-function/c any/c b (listof any/c)))
                     '(lift/c any/c listof (tail b)))
       (check-equal? (translate (binary-function/c b any/c (listof any/c)))
                     '(lift/c any/c listof (head b)))
       (check-equal? (translate (encoder/c sequence?))
                     'lift/c)
       (check-equal? (translate (encoder/c list?))
                     'lift/c)
       (check-equal? (translate (binary-function/c b (listof a) a))
                     '(reducer/c a (head b)))
       (check-equal? (translate (binary-function/c (listof a) b a))
                     '(reducer/c a (tail b)))
       (check-equal? (translate (binary-function/c b (listof any/c) any/c))
                     '(reducer/c (head b)))
       (check-equal? (translate (binary-function/c (listof any/c) b any/c))
                     '(reducer/c (tail b)))
       ...
       (check-equal? (translate (binary-function/c number? string? string?))
                     '(self-map/c string? (head number?)))
       (check-equal? (translate (binary-function/c string? number? string?))
                     '(self-map/c string? (tail number?)))
       (check-equal? (translate (binary-function/c number? list? list?))
                     '(map/c (head number?)))
       (check-equal? (translate (binary-function/c list? number? list?))
                     '(map/c (tail number?)))
       (check-equal? (translate (self-map/c list?))
                     'map/c)
       (check-equal? (translate (self-map/c (listof number?)))
                     '(map/c number?))
       (check-equal? (translate (self-map/c list? (tail number?)))
                     '(map/c (tail number?)))
       (check-equal? (translate (self-map/c (listof number?) (head symbol?)))
                     '(map/c number? (head symbol?)))
